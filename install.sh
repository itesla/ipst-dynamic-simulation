...
thirdparty_packs=$HOME/ipst_packs
...
usage() {
    ...
    echo "iPST options:"
    echo "  --help                   Display this help"
    echo "  --prefix                 Set the installation directory (default is $HOME/ipst)"
    ...
    echo "  --thirdparty-prefix      Set the thirdparty installation directory (default is $HOME/ipst_thirdparty)"
    echo "  --thirdparty-download    Sets false to compile thirdparty libraries from a local repository (default is true)"
    echo "  --thirdparty-packs       Sets the thirdparty libraries local repository (default is $HOME/ipst_packs)"
    echo ""
}
    ...
    if [ $ipst_package = true ]; then
        ...
        case "$ipst_package_type" in
            zip)
                ...
                $(cd "$sourceDir/distribution-dynamic-simulation/target/ipst-distribution-dynamic-simulation-${ipst_package_version}-full" && zip -rq "$sourceDir/${ipst_package_name}.zip" "ipst")
                zip -qT "${ipst_package_name}.zip" > /dev/null 2>&1 || exit $?
                ;;

            tar)
                [ -f "${ipst_package_name}.tar" ] && rm -f "${ipst_package_name}.tar"
                tar -cf "${ipst_package_name}.tar" -C "$sourceDir/distribution-dynamic-simulation/target/ipst-distribution-dynamic-simulation-${ipst_package_version}-full" . || exit $?
                ;;

            tar.gz | tgz)
                [ -f "${ipst_package_name}.tar.gz" ] && rm -f "${ipst_package_name}.tar.gz"
                [ -f "${ipst_package_name}.tgz" ] && rm -f "${ipst_package_name}.tgz"
                tar -czf "${ipst_package_name}.tar.gz" -C "$sourceDir/distribution-dynamic-simulation/target/ipst-distribution-dynamic-simulation-${ipst_package_version}-full" . || exit $?
                ;;

            tar.bz2 | tbz)
                [ -f "${ipst_package_name}.tar.bz2" ] && rm -f "${ipst_package_name}.tar.bz2"
                [ -f "${ipst_package_name}.tbz" ] && rm -f "${ipst_package_name}.tbz"
                tar -cjf "${ipst_package_name}.tar.bz2" -C "$sourceDir/distribution/target/ipst-distribution-dynamic-simulation-${ipst_package_version}-full" . || exit $?
                ;;
...
ipst_install()
{
    if [ $ipst_install = true ]; then
        ...
        cp -Rp "$sourceDir/distribution-dynamic-simulation/target/ipst-distribution-dynamic-simulation-${ipst_package_version}-full/ipst"/* "$ipst_prefix" || exit $?
    fi
}
